...
docs.generate:
	...
	go run -ldflags="-X main.Version=$(shell git describe --tags `git rev-list --tags --max-count=1`)" ./cli docs generate-project --project runtime/parser/schema/project.schema.json --rillyaml runtime/parser/schema/rillyaml.schema.json  --out docs/docs/reference/project-files
