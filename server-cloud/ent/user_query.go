...
func (uq *UserQuery) QueryGroups() *GroupsQuery {
	query := &GroupsQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithGroups(opts ...func(*GroupsQuery)) *UserQuery {
	query := &GroupsQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withGroups; query != nil {
		if err := uq.loadGroups(ctx, query, nodes,
			func(n *User) { n.Edges.Groups = []*Groups{} },
			func(n *User, e *Groups) { n.Edges.Groups = append(n.Edges.Groups, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withOrganization; query != nil {
		if err := uq.loadOrganization(ctx, query, nodes,
...
func (uq *UserQuery) loadProjects(ctx context.Context, query *ProjectQuery, nodes []*User, init func(*User), assign func(*User, *Project)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_projects" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadGroups(ctx context.Context, query *GroupsQuery, nodes []*User, init func(*User), assign func(*User, *Groups)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*User)
	nids := make(map[int]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(groups.FieldID), joinT.C(user.GroupsPrimaryKey[0]))
...
func (uq *UserQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*User, init func(*User), assign func(*User, *Organization)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.OrganizationTable)
		s.Join(joinT).On(s.C(organization.FieldID), joinT.C(user.OrganizationPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.OrganizationPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.OrganizationPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
