...
(async () => {
    ...
    await duckDbClient.execute(`CREATE TEMP TABLE AdBids_row as (select * from AdBids limit 1);`);
    await duckDbClient.execute(`CREATE OR REPLACE TEMPORARY VIEW FullTable AS (select * from AdBids b join Impressions i on b.id=i.id);`);
})();
