...
export class ActiveEventHandlerSpec extends TestBase {
    ...
    public setupSuite() {
        ...
        this.rillIntakeStub = this.sandbox.stub(RillIntakeClient.prototype, "fireEvent");
    }

    @TestBase.Test()
    public async shouldDelayTimer() {
        const {dataModelerStateService, focusCallback} =
            this.initTest(30 * 1000);
        ...
        expect(this.rillIntakeStub.firstCall.args[0]).toEqual(this.getExpectedActiveEvent(
            dataModelerStateService, 120000, "", 60, 1));
    }
    ...
    public async shouldHandleBlurAndFocusMultipleTimes() {
        ...
        expect(this.rillIntakeStub.firstCall.args[0]).toEqual(this.getExpectedActiveEvent(
            dataModelerStateService, 120999, "", 31, 2));
    }
    ...
    public async shouldFireMultipleTimes() {
        const {blurCallback, focusCallback, dataModelerStateService} = this.initTest();
        ...
        expect(this.rillIntakeStub.firstCall.args[0]).toEqual(this.getExpectedActiveEvent(
            dataModelerStateService, 120999, "", 31, 1));
        expect(this.rillIntakeStub.secondCall.args[0]).toEqual(this.getExpectedActiveEvent(
            dataModelerStateService, 240999, "", 31, 1));
    }
    ...
    private initTest(initialTime = 59999) {
        ...
        const {dataModelerStateService, activeEventHandler} = this.createActiveEventHandler();
        const blurCallback = this.windowListenerStub.args[0][1];
        const focusCallback = this.windowListenerStub.args[1][1];

        return {dataModelerStateService, activeEventHandler, blurCallback, focusCallback};
    }

    private createActiveEventHandler() {
        const dataModelerStateService = dataModelerStateServiceFactory(this.config);
        const activeEventHandler = new ActiveEventHandler(this.config,
