...
export class ExtractTableNameSpec extends TestBase {
    public tablePathTestData(): DataProviderData<[string, [string, string]]> {
        const getVariations = (fileName, expectedFileName,
    ...
    public shouldExtractAnSanitiseTableName(tablePath: string, [expectedTableName]: [string, string]) {
        expect(sanitizeTableName(extractTableName(tablePath))).toBe(expectedTableName);
    }

    @TestBase.Test("tablePathTestData")
    public shouldExtractExtension(tablePath: string, [,expectedExtension]: [string, string]) {
