...
Rill's [Databricks connector](/developers/build/connectors/data-source/databricks) now works with **Lakehouse//RT** real-time warehouses, in both live OLAP and ingestion modes. RT warehouses only speak the Statement Execution API and reject the Thrift protocol that SQL warehouses use by default, so Rill probes the warehouse on first connect and switches to the SEA backend automatically. No configuration is needed and existing Databricks SQL warehouses are unaffected; a `use_kernel` connector property is available to force SEA explicitly. The schema browser was also reworked to avoid a join that RT's Photon engine rejects.
