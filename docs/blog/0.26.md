...
- **Self-serve local project deployment with the CLI - ** [Deploying a project](/developers/deploy/from-local/) makes dashboards available online and enables you to invite others to access it without installing Rill locally. To try deploying your project `cd` into the project's root directory and run `rill deploy`.
