...
1. The templating engine first replaces expressions like `{{ .user.domain }}` with actual values ([Templating reference](../ingest/templating.md))
...
To support this, ingest a separate data [source](/ingest/connect) containing mappings of user email addresses to tenant IDs and reference it in the row-level filter. This can be a locally created csv file or any hosted data source.
