...
1. **Local AWS credentials** - Uses your local AWS configuration (from `~/.aws/credentials`)
   1. Note: This only a local only feature. 
...
6. For enhanced security: Select "Require external ID" and enter the External ID provided by Rill. This helps prevent the confused deputy problem.
