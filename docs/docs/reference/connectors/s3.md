...
When using IAM role-based authentication, you can configure the following parameters:

- **AWS Role ARN** (`aws_role_arn`) - The Amazon Resource Name (ARN) of the role to assume
- **AWS Role Session Name** (`aws_role_session_name`) - Optional name for the role session (defaults to "rill-session")
- **AWS External ID** (`aws_external_id`) - Optional external ID for cross-account access

### Setting Up an IAM Role for Rill

1. **Create an IAM Role**:
   - In the AWS Management Console, navigate to IAM > Roles > Create role
   - Select "Another AWS account" as the trusted entity
   - Enter your AWS account ID or the account ID that Rill will use
   - Optionally check "Require external ID" and provide a unique value that Rill will use
   - Attach policies that grant access to your S3 resources (like AmazonS3ReadOnlyAccess)
   - Name the role (e.g., "RillS3Access") and create it

2. **Configure Rill with the Role**:
   - When configuring S3 access in Rill, provide the role ARN in the format:
     `arn:aws:iam::ACCOUNT_ID:role/ROLE_NAME`
   - If you set an external ID, provide that as well
...
### How to create an IAM role for cross-account access

To set up an IAM role that Rill can assume for cross-account access:

1. Log in to the AWS Management Console of the account that owns the S3 bucket (the resource account).
...
5. Enter the AWS account ID that Rill will use to access the data (the trusted account).

6. Optional but recommended for security: Select "Require external ID" and create a unique identifier. Note this value as you'll need to provide it to Rill.
...
10. Give the role a descriptive name (e.g., "RillS3CrossAccountAccess") and an optional description.

11. Click "Create role".

12. After creating the role, click on it to view its details.

13. Note the "Role ARN" value which looks like: `arn:aws:iam::123456789012:role/RillS3CrossAccountAccess`

14. Configure Rill with:
    - Role ARN: The ARN from step 13
    - External ID: The value you created in step 6 (if applicable)
    - Role Session Name: Optional, defaults to "rill-session"
