...
Please see our reference documentation on [OLAP Engines](/developer/build/connectors/olap).
:::
...
2. [Environment](/developer/build/models/templating) level properties (e.g. a specific property that have been set for `dev`)
...
Primarily useful for [templating](/developer/build/connectors/templating), variables can be set in the `rill.yaml` file directly. This allows variables to be set for your projects deployed to Rill Cloud while still being able to use different variable values locally if you prefer. 
...
During development, it is always a good idea to check if your [access policies](/developer/build/metrics-view/security) are behaving the way you designed them to before pushing these changes into production. You can set mock users which enables a drop down in the dashboard preview to view as a specific user. 
