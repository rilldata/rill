...
_[object]_ - Defines an optional inline explore view for the metrics view. If not specified a default explore will be emitted unless `skip` is set to true.
  ...
  - **`ai_prompts`** - _[array of oneOf]_ - Suggested prompts shown as starters in the AI chat. Each entry is either a prompt string or an object with `label` and `prompt`. If set on a dashboard, the AI does not generate its own suggestions for it. At most 8 entries.
