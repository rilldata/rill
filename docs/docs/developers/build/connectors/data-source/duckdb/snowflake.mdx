...
Create `connectors/snowflake.yaml`:
...
```bash
SNOWFLAKE_DSN=myuser@myaccount/mydb/myschema?warehouse=mywh&role=myrole&authenticator=SNOWFLAKE_JWT&privateKey=<your_url_safe_private_key>
```
...
```bash
SNOWFLAKE_DSN=myuser:mypassword@myaccount/mydb/myschema?warehouse=mywh&role=myrole
```
...
Create `models/snowflake_data.yaml`:

```yaml
type: model
connector: snowflake
dev:
  sql: SELECT * FROM my_database.schema.my_table limit 10000

sql: SELECT * FROM my_database.schema.my_table
```
