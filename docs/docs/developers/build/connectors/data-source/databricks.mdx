...
configuration needed. (You can also force it with `use_kernel: true`.) SEA requires a
Rill build that links the `databricks_kernel` backend; otherwise the connection fails
with a clear error. Ordinary DBSQL warehouses keep using Thrift.
:::
