...
import { EnvPullTip, DeployToCloud } from '@site/src/components/connector';
...
To connect to Amazon S3, you can choose from five authentication options:
...
3. **WebIdentity Federation** (OIDC token file or GCP Workload Identity)
4. **Public** (for publicly accessible buckets - no authentication required)
5. **Local AWS credentials** (local development only - not recommended for production)
...
When you add data from S3 through the Rill UI, you'll see four authentication options:
...
- **GCP Workload Identity**: Configure the token audience and the AWS role that trusts the Google identity. An optional second AWS role can be assumed for cross-account access.

- **OIDC token file**: Configure a mounted token path and the AWS role that trusts its issuer. It supports the same optional second-role flow.
...
Local AWS credentials are only available through manual configuration. See [Method 5: Local AWS Credentials](#method-5-local-aws-credentials-local-development-only) for setup instructions.
:::
...
# Include this when the access key is temporary:
aws_access_token: "{{ .env.AWS_SESSION_TOKEN }}"
```
...
aws_access_key_id: "{{ .env.AWS_ACCESS_KEY_ID }}"
aws_secret_access_key: "{{ .env.AWS_SECRET_ACCESS_KEY }}"
# Include this when the source credentials are temporary:
aws_access_token: "{{ .env.AWS_SESSION_TOKEN }}"
...
AWS_SESSION_TOKEN=your_optional_source_session_token
AWS_ROLE_ARN=arn:aws:iam::123456789012:role/RillDataAccess
AWS_EXTERNAL_ID=your_external_id
```

The configured access keys sign the `sts:AssumeRole` request. You can instead set `allow_host_access: true` to use credentials from the host AWS credential chain.

Then, [create your first model](#create-your-first-model).

## Method 3: WebIdentity Federation

WebIdentity federation exchanges an OIDC token for temporary AWS credentials without requiring static AWS keys. Rill supports tokens from a mounted file and Google-signed workload identity tokens from the GCP metadata server.

Set `region` explicitly for AWS China, AWS GovCloud, or any deployment where the standard AWS region is not available from the host environment or shared AWS configuration.
...
gcp_workload_identity_audience: "{{ .env.GCP_WORKLOAD_IDENTITY_AUDIENCE }}"
aws_web_identity_role_arn: "{{ .env.AWS_WEB_IDENTITY_ROLE_ARN }}"
```

The AWS role identified by `aws_web_identity_role_arn` must trust the Google identity. Google service-account tokens include an `azp` claim, so AWS maps the token's configured `aud` value to the `accounts.google.com:oaud` condition key. Restrict the trust policy with `accounts.google.com:aud`, `accounts.google.com:oaud`, and `accounts.google.com:sub`.

### Mounted OIDC Token File
...
To use the federated role only as a bootstrap identity and assume a separate S3 access role, add:

```yaml
aws_role_arn: "{{ .env.AWS_TARGET_ROLE_ARN }}"
aws_role_session_name: rill-s3-access
aws_external_id: "{{ .env.AWS_EXTERNAL_ID }}"
```

The bootstrap role needs `sts:AssumeRole` permission for the target role, and the target role must trust the bootstrap role. For compatibility, configurations that specify a WebIdentity token source with only `aws_role_arn` continue to use that ARN as the direct federated role.

:::note DuckDB credential lifetime
Rill resolves the role chain before each DuckDB model or partition run and creates a temporary DuckDB secret. A single query running longer than the one-hour role-chain session cannot refresh those inlined credentials.
:::

## Method 4: Public Buckets
...
## Method 5: Local AWS Credentials (Local Development Only)
