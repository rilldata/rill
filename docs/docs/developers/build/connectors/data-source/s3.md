...
sql: |
  select * from read_parquet('s3://my-bucket/path/to/data/*.parquet')
```
...
sql: |
  select * from read_parquet('s3://my-public-bucket/path/to/data/*.parquet')
```
...
DuckDB supports reading various file formats directly from S3:
...
-- Read CSV files
select * from read_csv('s3://my-bucket/data/*.csv', auto_detect=true, ignore_errors=1, header=true)

-- Read JSON files
select * from read_json('s3://my-bucket/data/*.json', auto_detect=true, ignore_errors=1)
```
