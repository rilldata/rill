...
3. Using data from a dev/staging environment

### Limiting the source data to a smaller time range

There are different ways this can be achieved, and the method also depends heavily on the data source being used. For example, assuming we had a [BigQuery source](/connect/data-source/bigquery) created from the connectors UI:
```yaml
type: model
connector: bigquery
materialize: true
...
Instead of a general hard limit of 10000, which may give a subpar experience in the dashboard review, we can change the YAML to include a dev partition that filters on a time column instead:
...
Often, data in external [OLAP engines](/connect/olap) is quite large in size and requires different considerations to improve performance. At Rill, we manage clusters in the hundreds of TB, so we've included some tips below based on our experience.
