...
2. [**Failed to Load Dashboard**](#troubleshooting-row-access-filters): "This dashboard currently has no data to display. This may be due to access permissions."
...
| _not defined_                                 | ~~`access: '{{ has "partners" .user.groups }}'`~~ | `access: "'{{ .user.domain }}' == 'domain.com'"` (**overwrites** project default)  | `access: '{{ has "external" .user.groups }}'`                                     | Dashboard accessible to users in **usergroup**, "external" AND **domain** ending in "domain.com"                           |
