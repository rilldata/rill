...
Under the `partitions:` parameter, you will define the pattern in which your data is stored. Both SQL and glob patterns support [templating](/ingest/templating) and can be used to separate `dev` and `prod` instances. 
