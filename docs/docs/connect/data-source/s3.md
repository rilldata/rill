...
When using Rill Developer on your local machine (i.e., `rill start`), Rill will either use the credentials configured in your local environment using the AWS CLI or an [explicitly defined connector YAML](/reference/project-files/connectors#s3) file via the Add Data UI. 
...
In Rill Developer, Connect to S3 via Add Data. This will automatically create the s3.yaml file in your connectors/ folder and populate the .env file with `connector.s3.aws_access_key_id`and `connector.s3.aws_secret_access_key`.
...
This approach ensures that your AWS sources can authenticate consistently across both local development and cloud deployment environments.
