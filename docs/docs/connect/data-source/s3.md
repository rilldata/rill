...
<img src='/img/connect/data-sources/s3.png' class='rounded-gif' style={{width: '75%', display: 'block', margin: '0 auto'}}/>
<br />
...
For seamless deployment to Rill Cloud, you can configure AWS credentials directly in your project's `.env` file instead of relying solely on the `aws configure` command, which only configures credentials for local CLI usage.

Create or update your `.env` file with the following AWS credentials:

```env
aws_access_key_id=AKIA...
aws_secret_access_key=+s+cGC...
```
