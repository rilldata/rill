...
Another way is simply to limit the number of rows retrieved from your source by setting the `LIMIT` paramater.

```yaml
type: source
connector: "duckdb"
sql: SELECT * FROM read_parquet('s3://bucket/path/*.parquet') {{ if dev }} LIMIT 1000 {{ end }}
```
...
### Leveraging variables to apply a filter and row limit dynamically to a source
...
Furthermore, our `source.yaml` file contains the following SQL:
