...
Service tokens (also called service accounts) provide programmatic access to Rill Cloud for production systems, scheduled jobs, backend APIs, and other automated workflows. Unlike [user tokens](/user-guide/manage/user-tokens), service tokens persist even if the creating user is removed from the organization.
