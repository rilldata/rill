...
Similar to our [Explore dashboards](/user-guide/explore/dashboard-101), Canvas Dashboards also include a similar navigation bar to control the dashboard components.
...
<!-- - _**Alerts, Bookmarks and Sharing:**_ You can create an [alert](/user-guide/explore/alerts) by selecting the bell, customizing the default view of the dashboard (see `purple` box) to a predefined set of metrics, dimensions, and filters by selecting the [bookmark](/user-guide/explore/bookmarks.md), or share the dashboard ([internally by clicking the `Share` button](/user-guide/manage/user-management#admin-invites-user-from-rill-cloud) or [externally via Public URLs](/user-guide/explore/public-url.md)) . -->
...
If no Explore dashboard exists, and/or you don't have [permissions to view it](/developer/build/dashboards/customization#define-dashboard-access), no button will appear and is as designed.
