...
Rill supports connecting your data to both [DuckDB](/developer/build/connectors/olap/duckdb) and [ClickHouse](/developer/build/connectors/olap/clickhouse). However, there are still some features in development for managed ClickHouse. For more information see our [managed ClickHouse docs](/developer/build/connectors/olap/clickhouse#rill-managed-clickhouse). If you've still got questions, [contact our team](/contact) for more information and scheduled feature releases!
:::
...
On the other hand, if you need to analyze **100s of GB of data**, we would recommend using Managed ClickHouse. This will add some complexity ([staging tables](/developer/build/models/staging-models)), but will in turn provide better dashboard performance. 

If data leans either way, a good deciding factor for which OLAP engine to use is your **familiarity with their SQL syntax**. Whether you're [creating models](/developer/build/models/models-101#intermediate-processing) or using [arithmetic functions](/developer/build/metrics-view) in the metrics view, you'll need to utilize the engine's built-in functions.
