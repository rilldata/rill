...
Rill requires credentials to connect to remote data sources such as private buckets (S3, GCS, Azure), data warehouses (Snowflake, BigQuery), OLAP engines (ClickHouse, Apache Druid), or other DuckDB sources (MotherDuck). Please refer to the appropriate [connector](/developer/build/connectors) and [OLAP engine](/developer/build/connectors/olap) page for instructions to configure credentials accordingly.
...
- [Pushing and pulling credentials to / from Rill Cloud](/user-guide/manage/project-management/variables-and-credentials)
...
2. **Credentials passed in as variables** - When starting Rill Developer via `rill start --env key=value` (see [templating](/developer/build/connectors/templating) for more details)
3. **Credentials configured via CLI** - For [AWS](/developer/build/connectors/data-source/s3#local-aws-credentials-local-development-only) / [Azure](/developer/build/connectors/data-source/azure#azure-cli-authentication-local-development-only) / [Google Cloud](/developer/build/connectors/data-source/gcs#method-3-local-google-cloud-cli-credentials) - **NOT RECOMMENDED for production use**

For more details, please refer to the corresponding [connector](/developer/build/connectors) or [OLAP engine](/developer/build/connectors/olap) page.
...
Additional variables can then be usable and referenceable for [templating](/developer/build/connectors/templating) purposes in the local instance of your project. 
...
If not, follow the steps to deploy then configure your credentials via the CLI running [`rill env configure`](/developer/deploy/deploy-credentials).
...
For a detailed guide, see our [clone a project guide](/developer/guides/clone-a-project).
