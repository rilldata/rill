...
- ### _[Rill Managed OLAP + Data Ingestion (Default)](/developer/build/connectors/data-source)_:
  
  Use Rill's embedded **ClickHouse / DuckDB** (depending on size of data) as the OLAP engine and ingest data from external sources. Full Rill functionality is available with [some caveats](/developer/build/connectors/data-source#managed-olap-engine-caveats) depending on which embedded engine you select.
 
      :::tip Rill Defaults with DuckDB
      When starting Rill for the first time, Rill will auto-populate the connector with a `duckdb.yaml`. To use ClickHouse, create a managed ClickHouse connector by selecting "Add Data", then ClickHouse, and finally "Rill-managed ClickHouse" in the UI. For more information, see [Rill Managed ClickHouse](/developer/build/connectors/olap/clickhouse#rill-managed-clickhouse).
      :::

- ### _[Bring Your Own OLAP (BYO OLAP)](/developer/build/connectors/olap)_: 
  
  For large-scale datasets (100GB+) or existing [OLAP infrastructure](/developer/build/connectors/olap#what-is-olap), connect to existing **ClickHouse**, **Druid**, **Pinot**, or **MotherDuck** instances. Use Rill's "live connectors" to ingest data directly into your OLAP engines.
...
<div className="connector-icon-grid">
  ...
  <ConnectorIcon
    ...
    content="Distributed OLAP datastore for real-time analytics and business intelligence."
    link="/developer/build/connectors/olap/starrocks"
    linkLabel="Learn more"
...
Rill is continually evaluating additional OLAP engines that can be added. For a full list of OLAP engines that we support, refer to our [OLAP Engines](/developer/build/connectors/olap) page. If you don't see an OLAP engine that you'd like to use, please don't hesitate to [reach out](/contact)!
:::
