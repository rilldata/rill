...
Access policies are defined in the **metrics view** and/or **[dashboard YAML](/developer/build/dashboards/customization#define-dashboard-access)**.  
...
When a user loads a dashboard, the policies are resolved in two phases:
  1. The templating engine first replaces expressions like `{{ .user.domain }}` with actual values ([Templating reference](/developer/build/connectors/templating))
...
For more details, see [Service Tokens](/user-guide/manage/service-tokens).
:::
...
Access Policies assume that the user already has access to the project in Rill Cloud. For more information on user management, see our [User Management](/user-guide/manage/user-management) and [Project Management](/user-guide/manage/project-management) documentation.
...
[Learn more about security defaults →](/developer/build/project-configuration#metrics-views-security-policy)
:::
...
Dashboards also have an `access` key that can add additional security to the metrics view. Both [explore](/developer/build/dashboards/customization#define-dashboard-access) and [canvas](/developer/build/dashboards/customization#define-dashboard-access) dashboards can set the following:
...
3. Solve project access issues higher up in the [user](/user-guide/manage/user-management) / [usergroup](/user-guide/manage/usergroup-management) settings, and keep default project security rules.
...
[Learn more about testing security →](/developer/build/project-configuration#testing-security)
:::
...
When [requesting an embedded dashboard from Rill](/developer/integrate/embedding) from your frontend, you can pass the `attributes` parameter with custom names to ensure that the resulting dashboard displays the correct information.

For more information, see [our embedding docs](/developer/integrate/embedding#backend-build-an-iframe-url).
...
To support this, ingest a separate data [source](/developer/build/connectors) containing mappings of user email addresses to tenant IDs and reference it in the row-level filter. This can be a locally created CSV file or any hosted data source.
...
Another use case for row access policies is to ensure that your embedded dashboard provides a specific view for your end users. During the [embed dashboard request](/developer/integrate/embedding), you can pass custom attributes (other than the ones provided out-of-the-box) that map directly to a value within your Rill explore dashboard.
