...
Under the `partitions:` parameter, you will define the pattern in which your data is stored. Both SQL and glob patterns support [templating](/developer/build/connectors/templating) and can be used to separate `dev` and `prod` instances.
...
You will need to enable [incremental modeling](/developer/build/models/incremental-partitioned-models) in order to individually refresh a partition. 