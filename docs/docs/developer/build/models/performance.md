...
As discussed in the [templating section](/developer/build/models/templating), there are a few key recommendations to increase model performance in Rill Developer:

1. [Limiting the source data](/developer/build/models/templating#applying-a-one-week-sample-to-the-source-bucket-for-local-development) to a smaller time range (e.g., one week's worth of data instead of the full year)
2. Creating smaller models for development by [applying a raw limit](/developer/build/models/templating#example-conditional-sql-limiting-dev-rows), which will then serve as the starting point for your actual downstream models/modeling
3. [Applying a filter](/developer/build/models/templating#example-leveraging-variables-to-apply-a-filter-and-row-limit-dynamically-to-a-model) to the data to work with only a subset of data
...
Model materialization is something to consider when creating intermediate models. Other than [source models](/developer/build/models/source-models), intermediate models are not, by default, materialized and are views in your underlying database engine. There are some pros and cons to enabling it during the development process.
