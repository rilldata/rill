...
export class ActionQueueOrchestratorDisabled extends TestBase {
  ...
  public setup() {
    ...
    this.actionQueueOrchestrator = new ActionQueueOrchestrator(
      this.actionService
    );
  }
  ...
  public async shouldRunHigherPriorityActions() {
    ...
    await Promise.all([...promisesOne, ...promisesTwo]);
  ...
  public async shouldStopRunningCancelledQueries() {
    ...
    await Promise.all([...promisesOne, ...promisesTwo, ...promisesThree]);
  ...
  private async wrapEnqueue<Action extends keyof TestActionsDefinition>(
    ...
    action: keyof TestActionsDefinition,
    args: TestActionsDefinition[Action]
  ) {
    try {
      await this.actionQueueOrchestrator.enqueue(
        { id, priority },
        action,
        args
      );
