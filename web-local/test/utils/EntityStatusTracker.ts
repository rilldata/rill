...
export class EntityStatusTracker {
  ...
  public startTracker(entityType: EntityType) {
    ...
    this.stateServiceSpy.callsFake(async (...args) => {
      return this.originalStateService.apply(
    ...
    this.stateSpyTimer = setInterval(() => {
      ...
      if (
        entity &&
        this.statusChangeOrder[this.statusChangeOrder.length - 1] !==
          entity.status
      ) {
      ...
      const applicationState =
        this.dataModelerStateService.getApplicationState();
      if (
        this.applicationStatusChangeOrder[
          this.applicationStatusChangeOrder.length - 1
