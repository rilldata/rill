...
// S3 source ingestion test
// based on public bucket s3://playwright-s3-qa/*
// Can add more files as required, currently parquet.gz files are erroring so removed.

test.describe("LOAD DATA FROM cloud", () => {
  RillTest("Reading Source into Rill from S3", async ({ page }) => {
    ...
    await Promise.all([
      ...
      cloud(page, "customer_data.csv", "s3"),
    ]);
    ...
    await Promise.all([
      ...
      cloud(page, "AdImpressions_parquet.parquet", "s3"),
    ]);
    /*  broken
     await Promise.all([
       waitForTable(page, '/sources/AdImpressions_parquet_gz.yaml', AdImpressionsColumns),
       cloud(page, 'AdImpressions_parquet_gz.parquet.gz', 's3'),
     ]);
     */
    ...
    await Promise.all([
      ...
      cloud(page, "AdImpressions_tsv.tsv", "s3"),
    ]);
  });
});
