...
import { actionUsingMenu, renameFileUsingMenu, checkExistInConnector } from "../utils/commonHelpers";
...
test.describe('Check Source UI buttons.', () => {
    RillTest('Reading Source into Rill from GCS', async ({ page }) => {
        ...
        const resultsPreviewTableClose = await page.locator('[aria-label="Results Preview Table"]'); // #6316
        await expect(resultsPreviewTableClose).toBeHidden();

        await page.getByRole('button', { name: 'Toggle table visibility' }).click(); // #6308
        const resultsPreviewTableOpen = await page.locator('[aria-label="Results Preview Table"]'); // #6316
        await expect(resultsPreviewTableOpen).toBeVisible();

        await page.getByRole('button', { name: 'Toggle inspector visibility' }).click();  // #6308
        const inspectorPanelClose = await page.locator('[aria-label="Inspector Panel"]'); // #6316
        await expect(inspectorPanelClose).toBeHidden();

        await page.getByRole('button', { name: 'Toggle inspector visibility' }).click();  // #6308
        const inspectorPanelOpen = await page.locator('[aria-label="Inspector Panel"]'); // #6316
        ...
        await page.locator('div[role="menuitem"]:has-text("Create model")').waitFor();
        ...
        await page.getByRole('button', { name: 'Go to' }).click();
        await page.locator('div[role="menuitem"]:has-text("Create model")').waitFor();
        await expect(page.locator('a[role="menuitem"][href="/files/models/sales_model.sql"]')).toBeVisible();
        ...
        await renameFileUsingMenu(page, '/models/sales_model_1.sql', 'random_model.sql')
        ...
        await page.locator('a[role="menuitem"][href="/files/models/random_model.sql"]').click();
