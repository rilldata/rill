...
describe('cascadingContextStore', () => {
  ...
  it('generates derived values', () => {
    const store = cascadingContextStore('test1', { a: 10, b: 20}, { t1: (s) => s.a + s.b});
    const store2 = cascadingContextStore('test1', { a: 10, c: 30}, { t2: (s) => s.a + s.c});
    ...
    const store3 = cascadingContextStore('test1', {c: 40, d: 40}, { 
      t1: s => s.a + s.b, 
      t3: s => s.c + s.d
    });
    expect(get(store3)).toEqual({ a: 10, b: 20, c: 40, d: 40, t1: 30, t2: 40, t3: 80});
  })
})