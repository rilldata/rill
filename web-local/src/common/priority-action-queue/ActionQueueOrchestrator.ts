import type { ActionServiceBase } from "../ServiceBase";
import {
  PriorityActionQueue,
  QueuedActionArgsIdx,
  QueuedActionCallbacksIdx,
  QueuedActionNameIdx,
} from "./PriorityActionQueue";
import type { ActionMetadata } from "./PriorityActionQueue";
import { QueryCancelledError } from "../errors/QueryCancelledError";
...
> {
    ...
    while (queuedAction !== undefined) {
      try {
        const result = await this.actionService.dispatch(
          queuedAction[QueuedActionNameIdx],
          queuedAction[QueuedActionArgsIdx] as any
        );
  ...
  public clearQueue(id: string): void {
    ...
    queuedActions.forEach((queuedAction) =>
      queuedAction[QueuedActionCallbacksIdx].promiseReject(
