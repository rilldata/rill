...
const MOCK_CITIES = range(0, 1000, (i) => faker.location.city());
...
function allDimCombinations(dims: PivotDimension[], currentIndex: number) {
  ...
  const currentCombinations = MOCK_CITIES.slice(
    0,
    getDimensionCardinality(currentDim.def)
  );
  // singleDimCombinations(
...
const MOCK_PRODUCTS = range(0, 1000, (i) => faker.commerce.product());
const MOCK_COUNTRY = range(0, 1000, (i) => faker.location.country());
...
export function getFlatRowHeaders(config: PivotConfig, y0: number, y1: number) {
  ...
  let data = Array.from(Array(meta.rowCt).keys()).map((y) =>
    Array.from(Array(2).keys()).map(
      ...
      (x) =>
        `${x % 2 ? MOCK_COUNTRY[y] : MOCK_PRODUCTS[y]}${
          y % 2 === 0 ? "\u200B" : ""
        }`
    )
  );
...
export function getNestedRowHeaders(
  ...
  let parentIdx = 0;
  let data = [];
  let rowColumnCt = config.expanded.length > 0 ? 2 : 1;
  const makeHeader = (header: any[]) => {
    header.length = rowColumnCt;
    return header;
  };
