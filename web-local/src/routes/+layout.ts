...
export async function load({ url, depends, untrack, route }) {
  ...
  // Enforce CLI --preview route locking. When the CLI flag is set, the user
  // is locked into preview mode and cannot reach developer-only routes.
  // In non-CLI mode no locking is needed; mode is URL-driven via
  // previewModeStore so users can toggle freely between modes.
