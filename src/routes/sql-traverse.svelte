...
function scanMatches(str, location) {
    ...
    while ((match = re.exec(str)) != null) {
        matches.push({
            index: match.index, content: match[0]
        });
    }
    ...
    matches.forEach(({ index }) => {
        ...
        matchSet.push({
            substring, start: index, end: ri, length: substring.length
        })
        // let's scan the main string from the index.
    })

    let candidates = matchSet.filter(match => location >= match.start && location <= match.end);

    // get the smallest one, since that represents most nested (the most nested can't be a longer length than the next one up.)
    candidates.sort((a,b) => {
        return a.length - b.length;
    })
    let finalMatch = candidates.length ? candidates[0] : { substring: str, start: 0, end: str.length-1, length: str.length}
...
function getSubquery(str, location) {
    ...
    return {
        view: `${str.slice(0, final.start)}<span style="font-weight:bold; color: black;">${final.substring}</span>${str.slice(final.end+1)}`,
    }
...
function extractCTEs(string, location) {
    ...
    while (ri < string.length) {
        ...
        if (string.slice(si, ri).toLowerCase().endsWith(' as')) {
            currentExpression.name = string.slice(si, ri - 3).replace(',', '').trim();
        }
        ...
        if (char ===')' && nest === 0) {
            ...
            currentExpression.substring = string.slice(ei, ri+1).slice(1, -1).trim();
