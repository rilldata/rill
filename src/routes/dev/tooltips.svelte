...
function handleDrag(event) {
    // if (entered) {
    if (activated) {
        x = Math.max(20, Math.min(event.clientX - hoverElement.clientWidth / 2 + scrollX))
        y = Math.max(20, Math.min(event.clientY - hoverElement.clientHeight / 2 + scrollY))
    }
    // }
}
...
$: if ((location === 'top' || location === 'bottom') && (alignment === 'top' || alignment === 'bottom')) {
    alignment = 'center';
}

$: if ((location === 'left' || location === 'right') && (alignment === 'left' || alignment === 'right')) {
    alignment = 'center';
}
...
<div class='fixed p-3 bg-white border border-black border-3 rounded' style:right="20px" style:top="20px">
...
another element!
            ...
            </svelte:fragment></Tooltip> very popular during the Renaissance. The first line of Lorem Ipsum, 
...
<Tooltip distance={20} {location} {alignment}>
    <div 
        ...
        class="absolute rounded outline p-5 hover:outline-sky-500 hover:text-sky-800 hover:bg-sky-100 transition-colors hover:outline-4 hover:font-bold backdrop-blur-sm select-none" style:left="{x}px" style:top="{y}px" style:width="200px" 
        bind:this={hoverElement}
            on:mousedown={() => { entered = true; }} 
            on:mouseup={() => { entered = false; }}
            on:mouseleave={() => { 
    ...
    <svelte:fragment slot="tooltip-content">
        <!-- <TooltipContent> -->
            <div style="min-height: 300px; width: 300px;" class="border border-2 border-black p-3 rounded backdrop-blur-md	">
                This is a tooltip. It will follow our little 🚐 everywhere it goes.
                Ignore the design; the <i>positioning semantics</i> are the important part.
