<script>
  ...
  $: selects = content?.length
    ? extractCoreSelectStatements(content || "")
    : [];
  $: fromStatements = content?.length
    ? extractFromStatements(content || "")
    : [];
