...
$: currentExploreConfiguration = ($store && $store?.exploreConfigurations) ? $store?.exploreConfigurations.find(config => config.id === $store?.activeAsset?.id) : undefined;

$: preparedData = currentExploreConfiguration?.preview?.timeSeries.map(d => {
...
<section>
    ...
    <div style:grid-area="leaderboard-header">
        leaderboard header
    </div>
    ...
    <div style:grid-area="timeseries-body">
        ...
        {#each currentExploreConfiguration.activeMetrics as metric, i}
            <ExploreChart 
                ...
                xAxis={i === 0}
                bind:hoveredDate
                zeroBound={metric.function === 'count' || metric.function === 'approx_count_distinct'}
            />
        {/each}
        {/if}
    </div>
