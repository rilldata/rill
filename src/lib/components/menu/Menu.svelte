...
function handleKeydown(event) {
    ...
    if (key === 'ArrowDown') {
        $currentItem = $currentItem !== undefined ? Math.min($currentItem + 1, $menuItems.length - 1) : 0;
        
    }
    if (key === 'ArrowUp') {
        $currentItem = $currentItem !== undefined ? Math.max($currentItem - 1, 0) : 0;
    }
}
...
<div
    ...
    on:mouseleave={() => { $currentItem = undefined }}
    class='
        pt-2 
        pb-2
        shadow 
