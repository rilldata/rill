<script lang="ts">
    ...
    $: X = scaleLinear().domain([minX, maxX]).range([left + vizOffset, width - right - vizOffset]);

    $: yVals = data.map( d => d.count );
    $: maxY = Math.max(...yVals);
    $: Y = scaleLinear().domain([0, maxY]).range([height - buffer - bottom, buffer]);
    ...
    <svg {width} height={height} >
        ...
        <g shape-rendering=crispEdges>
            ...
            {/each}
            <line x1={X(X.domain()[0])} x2={width * $tw - right - vizOffset} y1={Y(0) + buffer} y2={Y(0) + 4} stroke={color} />
        </g>
        <slot 
            x={X} 
            y={Y}
        ...
        <!-- <g style:font-size="12px" class='textElements'>
            {#each [['min', min], ['25%', qlow], ['median', median], ['mean', mean], ['75%', qhigh], ['max', max]] as [label, value], i} 
                ...
                <text text-anchor="end" x={left - labelOffset} y={y}>
                        {label}
                </text>
                <text 
