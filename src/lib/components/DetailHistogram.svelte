<script lang="ts">
    ...
    <svg {width} {height} >
        ...
        <!-- <text in:fade font-weight=bold fill={color} font-size=11 x={X(data[0].low)} y={height - buffer + 12 - 3}>MIN</text> -->
        <text in:fade fill={color} font-size=11 x={X(data[0].low)}  y={height - buffer + 12 - 3}>{formattedLowValue}</text>

        <!-- <text in:fade fill={color} font-weight=bold text-anchor=end font-size=11 x={X(data.slice(-1)[0].high)} y={height - buffer + 12 - 3}>MAX</text> -->
        <text in:fade fill={color} text-anchor=end font-size=11 x={X(data.slice(-1)[0].high)}  y={height - buffer + 12 - 3}>{formattedHighValue}</text>
