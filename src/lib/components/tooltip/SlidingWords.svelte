 ...
 */
function hackedLocalKeyFly(node: Element, {
	delay = 0,
	easing = cubicOut,
	x = 0,
	y = 0,
	...
	return {
		...
		easing,
		css: (t, u) => `
			transform: ${transform} translate(${(1 - t) * x * (duration === 0 ? 0 : 1)}px, ${(1 - t) * y * (duration === 0 ? 0 : 1)}px);
...
<div class="relative">
    ...
    {#key active}
        <span class="absolute" style:left="0" style:top="0px" 
