...
func (s *Service) CreateProject(ctx context.Context, org *database.Organization, opts *database.InsertProjectOptions, deploy bool) (*database.Project, error) {
	...
	if opts.CreatedByUserID != nil {
		err = s.InsertProjectMemberUser(txCtx, org.ID, proj.ID, *opts.CreatedByUserID, adminRole.ID, nil, nil)
		if err != nil {
			return nil, err
		}
	}
	...
	if org.DefaultProjectRoleID != nil {
		err = s.DB.InsertProjectMemberUsergroup(txCtx, allMembers.ID, proj.ID, *org.DefaultProjectRoleID, nil)
		if err != nil {
			return nil, err
		}
	}
