...
func work(ctx context.Context, logger *zap.Logger, name string, fn func(context.Context) error) error {
	...
	start := time.Now()
	logger.Info("job started", zap.String("job_kind", name), observability.ZapCtx(ctx))
	err := fn(ctx)
	...
	if err != nil {
		logger.Error("job failed", zap.String("job_kind", name), zap.Error(err), zap.Duration("duration", time.Since(start)), observability.ZapCtx(ctx))
		return err
	}
	logger.Info("job completed", zap.String("job_kind", name), zap.Duration("duration", time.Since(start)), observability.ZapCtx(ctx))
	return nil
}
