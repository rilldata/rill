...
func (o *Orb) SetAutoTopUp(ctx context.Context, customerID string, enabled bool, threshold float64, amount float64) (*AutoTopUp, error) {
	if customerID == "" {
		return nil, ErrCustomerIDRequired
	}

	// First, delete any existing top-ups for this customer
	existing, err := o.client.Customers.Credits.TopUps.ListByExternalID(ctx, customerID, orb.CustomerCreditTopUpListByExternalIDParams{})
	if err != nil {
		var orbErr *orb.Error
		if errors.As(err, &orbErr) && orbErr.Status == orb.ErrorStatus404 {
			// Customer not found; nothing to delete
		} else {
			return nil, fmt.Errorf("failed to list existing top-ups: %w", err)
		}
	} else {
		for _, tu := range existing.Data {
			err = o.client.Customers.Credits.TopUps.DeleteByExternalID(ctx, customerID, tu.ID)
			if err != nil {
				return nil, fmt.Errorf("failed to delete existing top-up %s: %w", tu.ID, err)
			}
		}
	}
	...
	// Create new top-up
	resp, err := o.client.Customers.Credits.TopUps.NewByExternalID(ctx, customerID, orb.CustomerCreditTopUpNewByExternalIDParams{
		Amount:           orb.F(fmt.Sprintf("%.2f", amount)),
		Currency:         orb.F("USD"),
		Threshold:        orb.F(fmt.Sprintf("%.2f", threshold)),
		PerUnitCostBasis: orb.F("1.00"),
		InvoiceSettings: orb.F(orb.CustomerCreditTopUpNewByExternalIDParamsInvoiceSettings{
			...
			Memo:           orb.F("Auto-refill credits"),
		}),
	})
	if err != nil {
	...
	o.logger.Info("SetAutoTopUp: created",
		zap.String("customer_id", customerID),
		zap.String("top_up_id", resp.ID),
...
func (o *Orb) GetAutoTopUp(ctx context.Context, customerID string) (*AutoTopUp, error) {
	if customerID == "" {
		return nil, ErrCustomerIDRequired
	}
	...
	tu := existing.Data[0]
	var thresholdVal, amountVal float64
	fmt.Sscanf(tu.Threshold, "%f", &thresholdVal)
	fmt.Sscanf(tu.Amount, "%f", &amountVal)
