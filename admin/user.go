...
// It may be called with or without holding an existing transaction.
func (s *Service) InsertProjectMemberUser(ctx context.Context, orgID, projectID, userID, roleID string, resources []database.ResourceName) error {
...
func (s *Service) CreateOrUpdateUser(ctx context.Context, email, name, photoURL string) (*database.User, error) {
	...
	for _, invite := range projectInvites {
		...
		err = s.InsertProjectMemberUser(ctx, project.OrganizationID, invite.ProjectID, user.ID, invite.ProjectRoleID, invite.Resources)
		if err != nil {
			return nil, err
		}
