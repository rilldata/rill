...
// It may be called with or without holding an existing transaction.
func (s *Service) InsertProjectMemberUser(ctx context.Context, orgID, projectID, userID, roleID string, attributes map[string]interface{}, resources []database.ResourceName) error {
	...
	err = s.DB.InsertProjectMemberUser(ctx, projectID, userID, roleID, resources)
	if err != nil {
		return err
	}
...
func (s *Service) CreateOrUpdateUser(ctx context.Context, email, name, photoURL string) (*database.User, error) {
	...
	for _, invite := range projectInvites {
		...
		err = s.InsertProjectMemberUser(ctx, project.OrganizationID, invite.ProjectID, user.ID, invite.ProjectRoleID, nil, invite.Resources)
		if err != nil {
			return nil, err
		}
	...
	for _, whitelist := range projectWhitelistedDomains {
		...
		err = s.InsertProjectMemberUser(ctx, project.OrganizationID, whitelist.ProjectID, user.ID, whitelist.ProjectRoleID, nil, nil)
		if err != nil {
			return nil, err
		}
		addedToProjectIDs[project.ID] = true
