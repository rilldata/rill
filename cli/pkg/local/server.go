...
func (s *Server) DeployProject(ctx context.Context, r *connect.Request[localv1.DeployProjectRequest]) (*connect.Response[localv1.DeployProjectResponse], error) {
	...
	if r.Msg.Upload { // upload repo to rill managed storage instead of github
		ghRepo, err := c.CreateManagedGithubRepo(ctx, &adminv1.CreateManagedGithubRepoRequest{
		...
		err = gitutil.CommitAndForcePush(ctx, s.app.ProjectPath, ghRepo.RepoHtmlUrl, ghRepo.Token)
		if err != nil {
			return nil, err
		}
...
func (s *Server) RedeployProject(ctx context.Context, r *connect.Request[localv1.RedeployProjectRequest]) (*connect.Response[localv1.RedeployProjectResponse], error) {
	...
	if r.Msg.Reupload {
		...
		if projResp.Project.ArchiveAssetId != "" {
			...
			gitURL = projResp.Project.GithubUrl
			token = creds.GitPassword
		}

		err = gitutil.CommitAndForcePush(ctx, s.app.ProjectPath, gitURL, token)
		if err != nil {
			return nil, err
		}
		if projResp.Project.ArchiveAssetId != "" {
