...
func RevokeResourceCmd(ch *cmdutil.Helper) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "revoke-resource",
		Short: "Remove resource-level access previously granted to a user group",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_, err = client.RemoveProjectMemberUsergroupResources(cmd.Context(), &adminv1.RemoveProjectMemberUsergroupResourcesRequest{
				Org:       ch.Org,
				Project:   projectName,
				Usergroup: groupName,
				Resources: resources,
			...
			ch.PrintfSuccess("Removed resource access (%s) for user group %q in project \"%s/%s\"\n", resourceList, groupName, ch.Org, projectName)
			return nil
		},
	}
