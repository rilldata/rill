...
func DeploymentCreateCmd(ch *cmdutil.Helper) *cobra.Command {
	...
	createCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ch.PrintfBold("Creating dev deployment for branch %q...\n", branch)
			...
			ch.PrintfSuccess("Dev deployment created successfully!\n\n")
			ch.Printf("Deployment ID: %s\n", resp.Deployment.Id)
			ch.Printf("Branch: %s\n", resp.Deployment.Branch)
			ch.Printf("Status: %s\n", resp.Deployment.Status.String())
			if resp.Deployment.RuntimeHost != "" {
				ch.Printf("Runtime Host: %s\n", resp.Deployment.RuntimeHost)
			}
