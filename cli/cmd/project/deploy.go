...
func DeployUsingManagedGitHubFlow(ctx context.Context, ch *cmdutil.Helper, opts *DeployOpts) error {
	...
	if projResp == nil || projResp.Project.ArchiveAssetId != "" {
		ghRepo, err := adminClient.CreateManagedGithubRepo(ctx, &adminv1.CreateManagedGithubRepoRequest{
			Organization: ch.Org,
			Name:         opts.Name,
		})
		if err != nil {
		...
		repoURL = projResp.Project.GithubUrl
		token = creds.GitPassword
	}

	err = gitutil.CommitAndForcePush(ctx, localProjectPath, repoURL, token)
	if err != nil {
