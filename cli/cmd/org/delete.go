...
This operation cannot be undone. Use --force to skip confirmation.`,
  ...
  rill org delete myorg --force`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := client.ListProjectsForOrganization(cmd.Context(), &adminv1.ListProjectsForOrganizationRequest{Organization: name})
			if err != nil {
				return err
			}
			...
			for i, proj := range projects {
				...
				_, err := client.DeleteProject(cmd.Context(), &adminv1.DeleteProjectRequest{Organization: name, Project: proj})
				if err != nil {
					return err
				}
			...
			_, err = client.DeleteOrganization(cmd.Context(), &adminv1.DeleteOrganizationRequest{Organization: name})
			if err != nil {
				return err
			}
