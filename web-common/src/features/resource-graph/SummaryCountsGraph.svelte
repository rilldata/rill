<script lang="ts">
  ...
  export let activeToken: 'sources' | 'metrics' | 'models' | 'dashboards' | null = null;
  ...
  onDestroy(() => { try { ro?.disconnect(); } catch {}; ro = null; });
  ...
  $: visible = (resources || []).filter((r) => {
    ...
    return k === ResourceKind.Source || k === ResourceKind.Model || k === ResourceKind.MetricsView || k === ResourceKind.Explore;
  });
  function listFor(kind: ResourceKind): V1Resource[] {
    return visible
      .filter((r) => coerceResourceKind(r) === kind)
      .sort((a, b) => (a.meta?.name?.name || "").localeCompare(b.meta?.name?.name || ""));
  }
  ...
  function toSeed(kind: ResourceKind, name: string) {
    const k = kind === ResourceKind.MetricsView ? 'metrics'
      : kind === ResourceKind.Explore ? 'dashboard'
      : kind === ResourceKind.Model ? 'model'
      : 'source';
    return `${k}:${name}`;
  }
  ...
  function buildNodes(
    width: number,
    counts: { sources: number; metrics: number; models: number; dashboards: number },
  ...
  $: flowColorMode = ($themeControl === "dark" ? "dark" : "light") as 'dark' | 'light';
  ...
  <div class="canvas" bind:this={containerEl}>
    {#key flowKey}
      <SvelteFlow
        ...
        fitView
        fitViewOptions={{ padding: 0.25, minZoom: 0.25, maxZoom: 1.2, duration: 150 }}
        ...
        on:nodeClick={(e) => navigateTokenForNode(e.detail?.node?.id || e.detail?.id)}
      >
        <Background gap={20} />
      </SvelteFlow>
    {/key}
