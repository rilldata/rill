...
/**
 * Import flow for the vendored Vega-Lite example gallery.
 *
...
/** Lazily loads the vendored gallery snapshot (it is ~2 MB, so never load it eagerly). */
export async function loadGallery(): Promise<GallerySnapshot> {
  const module = await import("./vega-examples.json");
  return module.default as unknown as GallerySnapshot;
}

/**
 * Converts a gallery example into component YAML. By default the spec is
 * parameterized deterministically when its encodings can be analyzed, and imported
 * verbatim (with sample data) otherwise. With parameterize: false the example is
 * always imported verbatim — used when the AI conversion runs at import time and
 * needs the untouched example (including its sample values) as input. In both
 * cases fixed sizes are normalized to fill the container, since gallery examples
 * assume a standalone fixed-size render.
 */
export function exampleComponentYAML(
  example: GalleryExample,
  options: { parameterize?: boolean } = {},
): string {
  const spec = JSON.parse(example.spec) as Record<string, unknown>;
  normalizeSize(spec);

  const parameterized =
    options.parameterize === false ? null : parameterizeExampleSpec(spec);
...
export async function importVegaExample(
  ...
  await runtimeServicePutFile(client, {
    path: filePath,
    blob: exampleComponentYAML(example, options),
    create: true,
    createOnly: true,
  });
