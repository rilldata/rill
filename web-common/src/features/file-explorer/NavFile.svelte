...
<li
  ...
  class="w-full text-left pr-2 h-6 group flex justify-between gap-x-1 items-center hover:bg-accent"
  class:bg-accent={isCurrentFile}
>
  <a
    class="w-full truncate flex items-center gap-x-1 font-medium ui-copy"
    class:ui-copy-muted={isDotFile || isProtectedDirectory}
    href={`/files${filePath}`}
    {id}
