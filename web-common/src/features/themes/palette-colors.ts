...
import { generatePalette, DEFAULT_STEP_COUNT, DEFAULT_GAMMA } from "./color-generation";
...
export function setPaletteColors(
  ...
  if (type === "sequential") {
    if (colors.length > 9) throw new Error("Maximum of 9 sequential colors allowed");
    colors.forEach((color, index) => setSequentialColor(index + 1, color, scopeElement));
...
export function clearPaletteColor(
  ...
  if (type === "sequential") {
    if (index < 1 || index > 9) throw new Error("Sequential index must be between 1 and 9");
    root.style.removeProperty(`--color-sequential-light-${index}`);
    root.style.removeProperty(`--color-sequential-dark-${index}`);
  } else if (type === "diverging") {
    if (index < 1 || index > 11) throw new Error("Diverging index must be between 1 and 11");
