...
vi.mock("../../../runtime-client/v2/gen/runtime-service", () => ({
  ...
  runtimeServiceGenerateFile: vi.fn(),
}));
...
describe("mergeEnvVars", () => {
  ...
  it("merges env vars into existing .env content", async () => {
    queryClient.fetchQuery.mockResolvedValue({
      blob: "EXISTING_VAR=existing_value",
    });

    const result = await mergeEnvVars(mockClient, queryClient as never, {
  ...
  it("handles .env file not found", async () => {
  ...
  it("updates existing env var values", async () => {
    queryClient.fetchQuery.mockResolvedValue({
  ...
  it("handles empty envVars map", async () => {
    queryClient.fetchQuery.mockResolvedValue({
      blob: "EXISTING=value",
    });
  ...
  it("skips entries with empty keys or values", async () => {
    queryClient.fetchQuery.mockResolvedValue({ blob: "" });
  ...
  it("invalidates query cache before fetching", async () => {
    queryClient.fetchQuery.mockResolvedValue({ blob: "" });

    await mergeEnvVars(mockClient, queryClient as never, { KEY: "value" });
  ...
  it("re-throws non-file-not-found errors", async () => {
    const error = new Error("network error");
    queryClient.fetchQuery.mockRejectedValue(error);

    await expect(
      mergeEnvVars(mockClient, queryClient as never, { KEY: "value" }),
    ).rejects.toThrow("network error");
  });

  it("handles suffixed env var names from backend", async () => {
    queryClient.fetchQuery.mockResolvedValue({
