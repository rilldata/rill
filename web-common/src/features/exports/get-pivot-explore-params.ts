...
export function getPivotExploreParams(
  ...
  // TODO: this should be based on explore state directly instead
  ...
  switch (view) {
    ...
    case "tdd": {
      ...
      const timeDim = newUrlParams.has(ExploreStateURLParams.TimeGrain)
        ? "time." + newUrlParams.get(ExploreStateURLParams.TimeGrain)
        : "";
