...
<DropdownMenu.Root onOpenChange={handleOpenChange}>
  <DropdownMenu.Trigger asChild let:builder>
    <button
      ...
      class="grid place-items-center text-muted-foreground hover:bg-gray-200 w-6 h-6"
    ...
    {:else}
      {#each GROUP_ORDER as groupKey}
        {#if groupedConversations[groupKey] && groupedConversations[groupKey].length > 0}
          <DropdownMenu.Group>
            <DropdownMenu.Label class="px-1 text-xs text-muted-foreground">
              {groupKey}
            </DropdownMenu.Label>
