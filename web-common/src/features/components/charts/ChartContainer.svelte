<script lang="ts">
  ...
  let metricsViewProvider = $derived(
  ...
  let chartDataQuery = $derived(
    chartProvider.createChartDataQuery(client, filterStore, timeControlStore),
  );
  ...
  let exploreState = $derived.by(() => {
    ...
    const baseState = transformTimeAndFiltersToExploreState(
      $filterStore,
      $timeControlStore,
    );
    const pivotState = transformChartSpecToPivotState(
      $spec,
      $timeControlStore.timeGrain,
    );
