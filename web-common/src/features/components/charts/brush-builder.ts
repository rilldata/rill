...
export async function compileToBrushedVegaSpec(
  ...
  // Detect the temporal interval signal generated by Vega-Lite for the brush.
  // In Vega-Lite 6, the `brush` signal is a static computed expression (no `on`
  // clause), so it never re-fires when the selection changes. The reactive signal
  // is `brush_<fieldname>` (e.g. `brush_ts`, `brush_event_time`) — identify it by
  // finding the brush_ signal that is NOT in the known non-temporal set.
  const temporalBrushSignal =
    originalSignals.find(
      (s) =>
        s.name?.startsWith("brush_") &&
        !knownBrushSignals.has(s.name) &&
        s.name !== "brush_scale_trigger" &&
        s.name !== "brush_tuple_fields" &&
        s.name !== "brush_translate_anchor" &&
        s.name !== "brush_translate_delta" &&
        s.name !== "brush_zoom_anchor" &&
        s.name !== "brush_zoom_delta" &&
        s.name !== "brush_modify",
    )?.name ?? "brush_ts";
