...
import { setLeaderboardMeasureNames } from "@rilldata/web-common/features/dashboards/state-managers/actions/core-actions";
import {
...
export const AD_BIDS_SORT_DESC_BY_IMPRESSIONS: TestDashboardMutation = (
  ...
  setLeaderboardMeasureNames(mut, [AD_BIDS_IMPRESSIONS_MEASURE]);
...
export const AD_BIDS_SORT_ASC_BY_BID_PRICE: TestDashboardMutation = (mut) => {
  setLeaderboardMeasureNames(mut, [AD_BIDS_BID_PRICE_MEASURE]);
  setSortDescending(mut);
  toggleSort(mut, mut.dashboard.dashboardSortType);
};
