...
export interface AllExploreStoreType {
  entities: Record<string, ExploreState>;
}
const { update, subscribe } = writable({
  entities: {},
} as AllExploreStoreType);
...
const metricsViewReducers = {
  ...
  syncFromUrl(
    ...
    const partial = getExploreStateFromUrl(
      urlState,
      metricsView,
      explore,
      schema,
...
export const explorerStore: Readable<AllExploreStoreType> &
...
export function useExploreState(name: string): Readable<ExploreState> {
  return derived(explorerStore, ($store) => {
