...
import { useCanvasTimeFiltersVariant } from "@rilldata/web-common/features/dashboards/time-controls/test/canvas-time-filters-variant";
import {
...
// Tests time filter interactions on a canvas dashboard, where `CanvasProvider` resolves the canvas
// and the time filter bar writes to the canvas entity's time filter manager and the url.
//
// The test bodies live in `test/time-filters-suite.ts`, which the explore dashboard runs as well.
describe("CanvasTimeFilters", () => {
  const variant = useCanvasTimeFiltersVariant(hoistedPage);
