...
```
lib/i18n/
├── messages/en.json   # source translations (edit these)
...
- `messages/en.json` is the base locale and the source of truth for keys.
  English is currently the only configured locale; add more by listing them in
  `project.inlang/settings.json` and adding a `messages/{locale}.json` file.
...
2. Replace the literal in code with `m.key()` (or `m.key({ var })`).
3. `npm run build:i18n` (or rely on the Vite plugin in dev).
4. Run `npm run test -w web-common` and `npm run quality`.
