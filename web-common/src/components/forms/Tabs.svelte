...
<Tabs {value} onValueChange={handleValueChange} class="w-full">
  ...
  >
    {#each options as opt}
      <TabsTrigger
        value={opt.value}
        class="flex-1 h-7 rounded-[8px] py-2 border border-gray-200 text-sm text-fg-primary transition-all relative z-0
              data-[state=active]:bg-surface-container data-[state=active]:shadow-lg  data-[state=active]:z-10 data-[state=active]:outline-2 data-[state=active]:outline-gray-200
