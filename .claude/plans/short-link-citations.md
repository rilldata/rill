...
2. **Citations in forked conversations**: `ForkSession` clones messages with new IDs but copies content verbatim. Citation URLs in cloned messages still reference the original conversation and call IDs. This works for the forker (who has access to the original), but breaks if the fork is shared with a third party who lacks access to the source conversation.
   ...
   - **Stable citation ID (message column variant)**: Generate a `citation_id` at tool-call time and persist it on the result message (for example `ai_messages.citation_id` with a `(session_id, citation_id)` index). Links resolve by `{conversation_id, citation_id}`. Forks copy messages unchanged, so inherited citations remain valid without rewriting. Smaller migration, but the resolver still traverses messages and keeps citation concerns coupled to message storage.
   - **Stable citation ID (dedicated table variant)**: Generate a `citation_id` at tool-call time and persist a citation record mapping `{citation_id -> session_id, source_call_id, query payload (or reference), ACL context}`. Forks copy messages unchanged because links are ID-based indirections. Most extensible long-term model, but adds a new storage surface, resolver path, and lifecycle/ACL semantics.
