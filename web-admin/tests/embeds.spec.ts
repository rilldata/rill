...
test.describe("Embeds", () => {
  test("embeds should load", async ({ embedPage }) => {
    ...
    await expect(frame.getByRole("button", { name: "Advertising Spend Overall $1.30M" })).toBeVisible();
  });
  ...
  test("state is emitted for embeds", async ({ embedPage }) => {
    ...
    expect(
      logMessages.some((msg) => msg.includes("tz=UTC&f=advertiser_name+IN+('Instacart')"))
    ).toBeTruthy();
  });
  ...
  test("getState returns from embed", async ({ embedPage }) => {
    ...
    await embedPage.waitForTimeout(500);
    expect(
      logMessages.some((msg) => msg.includes(`{"id":1337,"result":{"state":"tz=UTC&f=advertiser_name+IN+('Instacart')"}}`))
    ).toBeTruthy();
  });
