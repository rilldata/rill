...
<div class="flex flex-col justify-center items-center h-3/5 space-y-6 m-auto">
  ...
  <div class="flex flex-col items-center space-y-2">
    <h1 class="text-lg font-semibold">
      {m.dashboards_errored_title()}
    </h1>
    <p class="text-fg-secondary text-base">
      <ProjectAccessControls {organization} {project}>
        <svelte:fragment slot="manage-project">
          {m.dashboards_errored_body_manage()}
        </svelte:fragment>
        <svelte:fragment slot="read-project">
          {m.dashboards_errored_body_read()}
        </svelte:fragment>
  ...
  <ProjectAccessControls {organization} {project}>
    <svelte:fragment slot="manage-project">
      <CtaButton
        ...
        >{m.dashboards_errored_view_status()}
      </CtaButton>
    </svelte:fragment>
    <svelte:fragment slot="read-project">
      <CtaButton variant="secondary" href={`/${organization}/${project}`}
        >{m.dashboards_errored_view_project()}
      </CtaButton>
    </svelte:fragment>
  </ProjectAccessControls>
  {#if !isEmbedded}
    <p class="text-fg-secondary">
      {m.dashboards_errored_help_prefix()}
      <!-- i18n-ignore: "Discord" is a brand name -->
      <a href="https://discord.gg/2ubRfjC7Rh">Discord</a>
    </p>
  {/if}
</div>
