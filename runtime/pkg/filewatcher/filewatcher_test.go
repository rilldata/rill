...
func TestWatch(t *testing.T) {
	...
	w, err := NewWatcher(dir, nil, zap.NewNop())
	require.NoError(t, err)
	...
	go func() {
		err := w.Subscribe(ctx, func(es []WatchEvent) {
			for _, e := range es {
				ch <- e
			}
		})
