...
func TestCompile(t *testing.T) {
	...
	passTests := []struct {
		...
		{
			"select pub, dom from ad_bids_metrics where timestamp > time_range_start('P7D') and timestamp <= time_range_end('P7D')",
			"SELECT (\"publisher\") AS \"pub\", (\"domain\") AS \"dom\" FROM \"ad_bids\" WHERE (((\"timestamp\") > ?) AND ((\"timestamp\") <= ?)) GROUP BY 1, 2",
			advancedMV,
			[]any{parseTestTime(t, "2022-03-23T00:00:00Z"), parseTestTime(t, "2022-03-30T00:00:00Z")},
		},
		{
