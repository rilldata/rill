...
func TestDruidMVDRestrictions(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "two IN filters on one dim are ambiguous and not narrowed; nested ANDs are still flattened",
			dimensions: dims("tags"),
			where:      and(and(in("tags", "a", "b"), eq("city", "NYC")), in("tags", "b")),
			want:       nil,
		},
		{
			name:       "a repeated IN filter also counts as two and is not narrowed",
			dimensions: dims("tags"),
			where:      and(in("tags", "a"), in("tags", "a")),
			want:       nil,
		},
		{
			name:       "two IN filters with an ILIKE: the regex still wins",
			dimensions: dims("tags"),
			where:      and(in("tags", "a"), ilike("tags", "%b%"), in("tags", "c")),
