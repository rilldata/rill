	...
	for _, rollup := range e.metricsView.Rollups {
		...
		if e.metricsView.TimeDimension != "" {
			...
			if hasTimeRange {
				...
				// This ensures a rollup isn't rejected when the query extends beyond
				// both the base table and rollup (e.g. "all time" queries).
...
// Since rollup tables have the same column names as the base table, the base
// measure expressions work directly against the rollup table.
