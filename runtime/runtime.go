...
func (r *Runtime) ResolveSecurity(instanceID string, attributes map[string]any, rules []*runtimev1.SecurityRule, mv *runtimev1.Resource) (*ResolvedSecurity, error) {
	inst, err := r.Instance(context.Background(), instanceID)
	if err != nil {
		return nil, err
	}
	return r.securityEngine.resolveSecurity(instanceID, inst.Environment, attributes, rules, mv)
}
