...
- **Referencing non-existant environment variables:** Only reference environment variables that are present in `.env` (returned in `env` from `project_status`). If you need the user to add a new environment variable (e.g. credentials for a connector), first make all the file changes that reference the environment variable, then navigate to the `.env` file and stop with a message asking the user to manually add the required environment variable(s).
...
- **Adding undocumented properties:** Never add properties to a YAML file that are not documented for that resource type. For example, do not add a `description` property to a resource type that does not support it. Only use properties that are explicitly in the resource schema.
{% if not .external %}
