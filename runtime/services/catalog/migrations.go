...
func (s *Service) createInStore(ctx context.Context, item *MigrationItem) error {
	...
	err = s.wrapMigrator(item.CatalogInFile, func() error {
		return migrator.Create(ctx, s.Olap, s.Repo, opts, item.CatalogInFile, s.InstID, s.logger)
	})
	if err != nil {
		return err
	}
...
func (s *Service) updateInStore(ctx context.Context, item *MigrationItem) error {
	...
	if item.Type == MigrationUpdate {
		...
		err = s.wrapMigrator(item.CatalogInFile, func() error {
			...
			return migrator.Update(ctx, s.Olap, s.Repo, opts, item.CatalogInStore, item.CatalogInFile, s.InstID, s.logger)
		})
		if err != nil {
			return err
		}
