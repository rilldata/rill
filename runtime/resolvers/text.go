...
func init() {
	runtime.RegisterResolver("text", newText, analyzeText)
}
...
func newText(ctx context.Context, opts *runtime.ResolverOptions) (runtime.Resolver, error) {
	...
	refs, err := analyzeTextRefs(ctx, opts.Runtime, opts.InstanceID, props, opts.Claims.UserAttributes)
	if err != nil {
		return nil, err
	}
...
func analyzeText(ctx context.Context, rt *runtime.Runtime, opts *runtime.ResolverAnalysisOptions) (*runtime.ResolverAnalysis, error) {
	...
	refs, err := analyzeTextRefs(ctx, rt, opts.InstanceID, props, opts.UserAttributes)
	if err != nil {
		return nil, err
	}
	return &runtime.ResolverAnalysis{Refs: refs}, nil
}

// analyzeTextRefs discovers embedded metrics SQL with stub template functions.
// Discovery remains best effort and never initializes executable resolvers.
func analyzeTextRefs(ctx context.Context, rt *runtime.Runtime, instanceID string, props *textProps, userAttributes map[string]any) ([]*runtimev1.ResourceName, error) {
	...
	_, _ = parser.ResolveTemplate(props.Text, td, false)

	seen := make(map[string]bool)
	for _, sql := range sqls {
		res, err := rt.AnalyzeResolver(ctx, "metrics_sql", &runtime.ResolverAnalysisOptions{
	...
	for name := range seen {
		refs = append(refs, &runtimev1.ResourceName{Kind: runtime.ResourceKindMetricsView, Name: name})
	}
	return normalizeRefs(refs), nil
}
