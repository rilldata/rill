...
type metricsTimestampsResolver struct {
	runtime    *runtime.Runtime
	instanceID string
	mvName     string
	mv         *runtimev1.MetricsViewSpec
	executor   *executor.Executor
	args       *metricsTimestampsResolverArgs
}

type metricsTimestampsResolverProps struct {
	MetricsView string `mapstructure:"metrics_view"`
}

type metricsTimestampsResolverArgs struct {
...
// newMetricsTimestampsResolver returns physical data coverage (min/max timestamps) with open security.
...
func newMetricsTimestampsResolver(ctx context.Context, opts *runtime.ResolverOptions) (runtime.Resolver, error) {
	if !opts.Claims.SkipChecks && !opts.Claims.Admin() {
		return nil, errors.New("must be an admin to query metrics timestamps")
	}

	props := &metricsTimestampsResolverProps{}
	...
	// Use open security (no row filter); timestamps are about physical data coverage
	...
	return &metricsTimestampsResolver{
...
func (r *metricsTimestampsResolver) Close() error {
	r.executor.Close()
	return nil
}

func (r *metricsTimestampsResolver) CacheKey(ctx context.Context) ([]byte, bool, error) {
...
func (r *metricsTimestampsResolver) Refs() []*runtimev1.ResourceName {
	return []*runtimev1.ResourceName{{Kind: runtime.ResourceKindMetricsView, Name: r.mvName}}
}

func (r *metricsTimestampsResolver) Validate(ctx context.Context) error {
	return nil
}

func (r *metricsTimestampsResolver) ResolveInteractive(ctx context.Context) (runtime.ResolverResult, error) {
...
func (r *metricsTimestampsResolver) ResolveExport(ctx context.Context, w io.Writer, opts *runtime.ResolverExportOptions) error {
	return errors.New("not implemented")
}

func (r *metricsTimestampsResolver) InferRequiredSecurityRules() ([]*runtimev1.SecurityRule, error) {
