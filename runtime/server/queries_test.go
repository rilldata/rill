...
func prepareOLAPStore(t *testing.T) drivers.OLAPStore {
	conn, err := drivers.Open(context.Background(), "duckdb", "", "default", map[string]any{"dsn": ":memory:?access_mode=read_write", "pool_size": 4}, storage.MustNew(t.TempDir(), nil), activity.NewNoopClient(), zap.NewNop())
	require.NoError(t, err)
	olap, ok := conn.AsOLAP("")
	require.True(t, ok)
	return olap
