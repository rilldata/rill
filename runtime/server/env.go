...
func (s *Server) PullEnv(ctx context.Context, req *runtimev1.PullEnvRequest) (*runtimev1.PullEnvResponse, error) {
	...
	// Total count of cloud variables (returned regardless of whether anything is modified)
	totalCount := int32(0)
	...
	// Write merged variables per environment: local first, cloud overlays.
	for env, cloudVars := range cloudPerEnv {
		...
		if err := repo.Put(ctx, filepath.Join(root, envFileName), strings.NewReader(contents)); err != nil {
			return nil, fmt.Errorf("failed to write %q: %w", envFileName, err)
		}
		if _, err := gitutil.EnsureGitignoreHas(ctx, repo, envFileName); err != nil {
