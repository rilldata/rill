...
func (s *Server) GenerateMetricsViewFile(ctx context.Context, req *runtimev1.GenerateMetricsViewFileRequest) (*runtimev1.GenerateMetricsViewFileResponse, error) {
	...
	if data == "" {
		data, err = generateMetricsViewYAMLSimple(req.Connector, tbl, isDefaultConnector, modelFound, olap.Dialect())
		if err != nil {
			return nil, err
		}
	}
...
func generateMetricsViewYAMLSimple(connector string, tbl *drivers.OlapTable, isDefaultConnector, isModel bool, dialect drivers.Dialect) (string, error) {
	doc := &metricsViewYAML{
		Version:       1,
		Type:          "metrics_view",
