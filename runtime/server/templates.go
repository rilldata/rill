...
func (s *Server) GenerateFile(ctx context.Context, req *runtimev1.GenerateFileRequest) (*runtimev1.GenerateFileResponse, error) {
	...
	// Read the existing .env so the renderer can resolve env-var name conflicts.
	// A failure here is non-fatal; we surface a warning and proceed with an empty set.
	...
	if err == nil {
		...
		s.logger.Warn("templates: failed to open repo for .env conflict resolution; env var conflicts may not be detected", zap.Error(err))
	}
...
// .env is written first so YAML files referencing {{ .env.VAR }} have their secrets available.
func (s *Server) writeRenderedFiles(ctx context.Context, instanceID string, result *templates.RenderOutput) error {
	...
	if len(result.EnvVars) > 0 {
		...
		if err != nil && !errors.Is(err, fs.ErrNotExist) {
			s.logger.Warn("templates: failed to read .env; existing env vars may be overwritten", zap.Error(err))
		}
...
// Values are sanitized: newlines are stripped to prevent injection of additional
// env vars, and values containing whitespace or shell-special characters are quoted.
