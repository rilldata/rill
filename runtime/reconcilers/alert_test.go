		...
		Resource: &runtimev1.Resource_Alert{
			Alert: &runtimev1.Alert{
				Spec: &runtimev1.AlertSpec{
					...
					DataProperties:         must(structpb.NewStruct(map[string]any{"sql": "select * from bar where country <> 'Denmark'"})),
					NotifyOnRecover:        false,
					NotifyOnFail:           true,
					NotifyOnError:          false,
					Renotify:               false,
		...
		Resource: &runtimev1.Resource_Alert{
			Alert: &runtimev1.Alert{
				Spec: &runtimev1.AlertSpec{
					...
					DataProperties:       must(structpb.NewStruct(map[string]any{"metrics_sql": "select measure_0 from mv1 where country <> 'Denmark' having measure_0 > 0"})),
					NotifyOnRecover:      false,
					NotifyOnFail:         true,
					NotifyOnError:        false,
					Renotify:             false,
