...
func (q *MetricsViewTimeSeries) buildClickHouseSQL(mv *runtimev1.MetricsViewSpec, tsAlias string, selectCols []string, whereClause, havingClause, timezone string) string {
	...
	if shift == "" {
		sql = fmt.Sprintf(
			`
					SELECT
					toTimeZone(date_trunc('%[1]s', toTimeZone(%[2]s::TIMESTAMP, '%[7]s')), '%[7]s') as %[3]s,
					%[4]s
					FROM %[5]s
					WHERE %[6]s
					GROUP BY %[3]s
		...
		sql = fmt.Sprintf(
			`
				SELECT
					toTimeZone(date_trunc('%[1]s', toTimeZone(%[2]s::TIMESTAMP, '%[7]s') + INTERVAL %[8]s) - (INTERVAL %[8]s), '%[7]s') as %[3]s,
				%[4]s
				FROM %[5]s
				WHERE %[6]s
				GROUP BY %[3]s
