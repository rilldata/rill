...
func (q *ColumnCardinality) Resolve(ctx context.Context, rt *runtime.Runtime, instanceID string, priority int) error {
	...
	case drivers.DialectDuckDB:
		requestSQL = fmt.Sprintf("SELECT approx_count_distinct(%s) AS count FROM %s", safeName(q.ColumnName), fullTableName(q.Database, q.DatabaseSchema, q.TableName))
	case drivers.DialectClickHouse:
		requestSQL = fmt.Sprintf("SELECT uniq(%s) AS count FROM %s", safeName(q.ColumnName), fullTableName(q.Database, q.DatabaseSchema, q.TableName))
	default:
