...
func (q *MetricsViewTimeRange) Resolve(ctx context.Context, rt *runtime.Runtime, instanceID string, priority int) error {
	...
	case drivers.DialectDuckDB:
		return q.resolveDuckDB(ctx, olap, q.MetricsView.TimeDimension, fullMetricsViewTableName(q.MetricsView), policyFilter, priority)
	case drivers.DialectDruid:
		return q.resolveDruid(ctx, olap, q.MetricsView.TimeDimension, fullMetricsViewTableName(q.MetricsView), policyFilter, priority)
	case drivers.DialectClickHouse:
		return q.resolveClickHouse(ctx, olap, q.MetricsView.TimeDimension, fullMetricsViewTableName(q.MetricsView), policyFilter, priority)
	default:
		return fmt.Errorf("not available for dialect '%s'", olap.Dialect())
	}
}

func (q *MetricsViewTimeRange) resolveDuckDB(ctx context.Context, olap drivers.OLAPStore, timeDim, fullTableName, filter string, priority int) error {
	if filter != "" {
...
func (q *MetricsViewTimeRange) resolveDruid(ctx context.Context, olap drivers.OLAPStore, timeDim, fullTableName, filter string, priority int) error {
	if filter != "" {
		filter = fmt.Sprintf(" WHERE %s", filter)
	}
...
func (q *MetricsViewTimeRange) resolveClickHouse(ctx context.Context, olap drivers.OLAPStore, timeDim, fullTableName, filter string, priority int) error {
	if filter != "" {
		filter = fmt.Sprintf(" WHERE %s", filter)
	}
