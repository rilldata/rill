...
func Ignore_TestDriver_types(t *testing.T) {
	driver := &driver{}
	handle, err := driver.Open(context.Background(), "", "default", map[string]any{"pool_size": 2, "dsn": "http://localhost:8888/druid/v2/sql"}, storage.MustNew(t.TempDir(), nil), activity.NewNoopClient(), zap.NewNop())
