...
func (e *selfToSelfExecutor) Execute(ctx context.Context, opts *drivers.ModelExecuteOptions) (*drivers.ModelResult, error) {
	...
	if !opts.IncrementalRun {
		...
		// Create the table
		err := olap.CreateTableAsSelect(ctx, stagingTableName, asView, inputProps.SQL, inputProps.PreInitSQL, inputProps.PostInitSQL, nil)
		if err != nil {
		...
		err := olap.InsertTableAsSelect(ctx, tableName, inputProps.SQL, inputProps.PreInitSQL, inputProps.PostInitSQL, false, true, outputProps.IncrementalStrategy, outputProps.UniqueKey)
		if err != nil {
