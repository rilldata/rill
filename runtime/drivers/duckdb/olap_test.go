...
func prepareConn(t *testing.T) drivers.Handle {
	conn, err := Driver{}.Open(context.Background(), "", "default", map[string]any{}, storage.MustNew(t.TempDir(), nil), activity.NewNoopClient(), zap.NewNop())
	require.NoError(t, err)
...
func Test_safeSQLString(t *testing.T) {
	...
	conn, err := Driver{}.Open(context.Background(), "", "default", map[string]any{"data_dir": path}, storage.MustNew(tempDir, nil), activity.NewNoopClient(), zap.NewNop())
	require.NoError(t, err)
