...
func Test_specialCharInPath(t *testing.T) {
	...
	conn, err := Driver{}.Open(context.Background(), "", "default", map[string]any{"init_sql": fmt.Sprintf("ATTACH %s", safeSQLString(dbFile))}, storage.MustNew(t.TempDir(), nil), activity.NewNoopClient(), zap.NewNop())
	require.NoError(t, err)
