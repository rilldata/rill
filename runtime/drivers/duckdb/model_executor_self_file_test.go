...
func TestExecute(t *testing.T) {
	tempDir := t.TempDir()
	duckDB, err := drivers.Open(context.Background(), "duckdb", "", "default", map[string]any{}, storage.MustNew(t.TempDir(), nil), activity.NewNoopClient(), zap.NewNop())
	...
	fileHandle, err := drivers.Open(context.Background(), "file", "", "default", map[string]any{}, storage.MustNew(tempDir, nil), activity.NewNoopClient(), zap.NewNop())
	require.NoError(t, err)
