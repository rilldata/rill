...
func (s *sqlStoreToDuckDB) transferFromRowIterator(ctx context.Context, iter drivers.RowIterator, table string) error {
	...
	defer func() {
		...
		_, err = conn.ExecContext(context.Background(), fmt.Sprintf("DROP TABLE IF EXISTS %s", safeName(tmpTable)))
		if err != nil {
	...
	return rwConn.CreateTableAsSelect(ctx, table, fmt.Sprintf("SELECT * FROM %s", safeName(tmpTable)), nil)
}
