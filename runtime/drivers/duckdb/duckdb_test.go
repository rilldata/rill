...
func TestNoFatalErr(t *testing.T) {
	...
	handle, err := Driver{}.Open(context.Background(), "", "default", map[string]any{"pool_size": 2}, storage.MustNew(t.TempDir(), nil), activity.NewNoopClient(), zap.NewNop())
...
func TestDuckDBModeEnforcement(t *testing.T) {
	...
	t.Run("read mode blocks model execution", func(t *testing.T) {
		handle, err := drivers.Open(context.Background(), "duckdb", "", "test", map[string]any{
			"dsn":  ":memory:",
			"mode": "read",
