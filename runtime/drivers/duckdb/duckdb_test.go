...
func TestFatalErrConcurrent(t *testing.T) {
	...
	go func() {
		qry := `
			...
			WHERE d.d2 IN ('');
		`
		err1 = olap.WithConnection(context.Background(), 0, func(ctx, ensuredCtx context.Context, _ *sql.Conn, _ driver.Conn) error {
	...
	go func() {
		qry := `SELECT * FROM a;`
		err2 = olap.WithConnection(context.Background(), 0, func(ctx, ensuredCtx context.Context, _ *sql.Conn, _ driver.Conn) error {
	...
	go func() {
		...
		qry := `SELECT * FROM a;`
		err3 = olap.WithConnection(context.Background(), 0, func(ctx, ensuredCtx context.Context, _ *sql.Conn, _ driver.Conn) error {
			return olap.Exec(ctx, &drivers.Statement{Query: qry})
		})
		wg.Done()
	}()
