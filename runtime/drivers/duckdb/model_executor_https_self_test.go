...
func TestHTTPToDuckDBTransfer(t *testing.T) {
	...
	to, err := drivers.Open(context.Background(), "duckdb", "", "default", map[string]any{}, storage.MustNew(t.TempDir(), nil), activity.NewNoopClient(), zap.NewNop())
	require.NoError(t, err)

	inputHandle, err := drivers.Open(context.Background(), "https", "", "default", map[string]any{}, storage.MustNew(t.TempDir(), nil), activity.NewNoopClient(), zap.NewNop())
	require.NoError(t, err)
