...
type OpenDataBucketFn func(ctx context.Context, instanceID string, elem ...string) (*blob.Bucket, error)

var OpenNilDataBucket OpenDataBucketFn = func(ctx context.Context, instanceID string, elem ...string) (*blob.Bucket, error) { return nil, nil }
...
func Open(driver, instanceID string, config map[string]any, storage *storage.Client, activity *activity.Client, logger *zap.Logger) (Handle, error) {
	d, ok := Drivers[driver]
	if !ok {
		return nil, fmt.Errorf("unknown driver: %s", driver)
	}

	conn, err := d.Open(instanceID, config, storage, activity, logger)
	if err != nil {
		return nil, err
	}
...
type Driver interface {
	...
	// If instanceID is empty, the connection is considered shared and its As...() functions may be invoked with different instance IDs.
	Open(instanceID string, config map[string]any, storage *storage.Client, activity *activity.Client, logger *zap.Logger) (Handle, error)
