...
func TestInformationSchema(t *testing.T) {
	...
	t.Run("testInformationSchemaSystemAllLike", func(t *testing.T) {
		conn, err := drivers.Open(context.Background(), "clickhouse", "", "default", map[string]any{"dsn": dsn + "/system", "mode": "readwrite"}, storage.MustNew(t.TempDir(), nil), activity.NewNoopClient(), zap.NewNop())
		require.NoError(t, err)
