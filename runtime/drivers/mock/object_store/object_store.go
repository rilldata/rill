...
func (h *handle) ListObjectsForGlob(ctx context.Context, bucket, glob string, pageSize uint32, pageToken string, startAfter string) ([]drivers.ObjectStoreEntry, string, error) {
	blobBucket, err := rillblob.NewBucket(h.bucket, h.logger)
	if err != nil {
		return nil, "", err
	}
	defer blobBucket.Close()
	return blobBucket.ListObjectsForGlob(ctx, glob, pageSize, pageToken, startAfter)
}
