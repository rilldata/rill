...
func (c *catalogStore) FindAISessions(ctx context.Context, ownerID, userAgentPattern string) ([]*drivers.AISession, error) {
	query := `
		SELECT id, instance_id, owner_id, title, user_agent,  checkpoint_message_id, created_on, updated_on
		FROM ai_sessions
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.InstanceID, &s.OwnerID, &s.Title, &s.UserAgent, &s.CheckpointMessageID, &s.CreatedOn, &s.UpdatedOn); err != nil {
			return nil, err
		}
...
func (c *catalogStore) FindAISession(ctx context.Context, sessionID string) (*drivers.AISession, error) {
	...
	var s drivers.AISession
	if err := row.Scan(&s.ID, &s.InstanceID, &s.OwnerID, &s.Title, &s.UserAgent, &s.CheckpointMessageID, &s.CreatedOn, &s.UpdatedOn); err != nil {
		return nil, err
	}
	return &s, nil
}
...
func (c *catalogStore) InsertAISession(ctx context.Context, s *drivers.AISession) error {
	_, err := c.db.ExecContext(ctx, `
		INSERT INTO ai_sessions (id, instance_id, owner_id, title, user_agent, checkpoint_message_id, created_on, updated_on)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, s.ID, s.InstanceID, s.OwnerID, s.Title, s.UserAgent, s.CheckpointMessageID, s.CreatedOn, s.UpdatedOn)
	if err != nil {
		return err
	}
	return nil
...
func (c *catalogStore) UpdateAISession(ctx context.Context, s *drivers.AISession) error {
	now := time.Now()
	_, err := c.db.ExecContext(ctx, `
		UPDATE ai_sessions SET owner_id = ?, title = ?, user_agent = ?, checkpoint_message_id = ?, updated_on = ?
		WHERE id = ?
	`, s.OwnerID, s.Title, s.UserAgent, s.CheckpointMessageID, now, s.ID)
	if err != nil {
		return err
	}
	s.UpdatedOn = now
