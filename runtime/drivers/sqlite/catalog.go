...
func (c *catalogStore) FindAISessions(ctx context.Context, ownerID, userAgentPattern string) ([]*drivers.AISession, error) {
	query := `
		SELECT id, instance_id, owner_id, title, user_agent, shared_until_message_id, created_on, updated_on
		FROM ai_sessions
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.InstanceID, &s.OwnerID, &s.Title, &s.UserAgent, &s.SharedUntilMessageID, &s.CreatedOn, &s.UpdatedOn); err != nil {
			return nil, err
		}
...
func (c *catalogStore) FindAISession(ctx context.Context, sessionID string) (*drivers.AISession, error) {
	...
	var s drivers.AISession
	if err := row.Scan(&s.ID, &s.InstanceID, &s.OwnerID, &s.Title, &s.UserAgent, &s.SharedUntilMessageID, &s.CreatedOn, &s.UpdatedOn); err != nil {
		return nil, err
	}
	return &s, nil
}
