...
func (c *connection) Query(ctx context.Context, props map[string]any) (drivers.RowIterator, error) {
	...
	if srcProps.DSN != "" { // get from src properties
		...
		return nil, fmt.Errorf("the property 'dsn' is required for Trino. Provide 'dsn' in the YAML properties or pass '--env connector.trino.dsn=...' to 'rill start'")
	}
...
func parseSourceProperties(props map[string]any) (*sourceProperties, error) {
	...
	if conf.SQL == "" {
		return nil, fmt.Errorf("property 'sql' is mandatory for connector \"trino\"")
	}
	return conf, err
}
